"""
Psychic Fortnight — single-page Streamlit UI.
Run with:
...
for key, default in [
    ("chat_history", []),
    ("pipeline_stage", "idle"),
    ("pending_query", None),        # query to process on next render
]:
...
with tab_query:
    ...
    # Example query buttons — set pending_query and rerun
    examples = [
    ...
    # Resolve query — either from chat_input or from pending_query
    ...
    if active_query:
        ...
        # No rerun needed — Streamlit will naturally continue rendering
    ...
    if not paths:
        ...
        if not graph_file.exists():
            ...
            if nodes:
                ...
                type_counts = Counter(
                    n.get("type", n.get("artifact_type", "unknown"))
                    for n in nodes
                )
